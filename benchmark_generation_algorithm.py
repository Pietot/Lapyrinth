...
def kruskal_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).kruskal(), number=1, globals=globals()), 5
    )
...
def prim_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).prim(), number=1, globals=globals()), 5
    )
...
def randomized_depth_first_search_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).randomized_depth_first_search(),
            number=1,
            globals=globals(),
        ),
        5,
...
def hunt_and_kill_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).hunt_and_kill(), number=1, globals=globals()),
        5,
    )
...
def eller_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).eller(), number=1, globals=globals()), 5
    )
...
def recursive_division_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: RecursiveMaze(size).recursive_division(),
            number=1,
            globals=globals(),
        ),
        5,
...
def binary_tree_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).binary_tree(), number=1, globals=globals()), 5
    )
...
def sidewinder_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).sidewinder(), number=1, globals=globals()), 5
    )
...
def growing_tree_new_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).growing_tree(mode="newest"), number=1, globals=globals()
        ),
        5,
    )
...
def growing_tree_mid_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).growing_tree(mode="middle"), number=1, globals=globals()
        ),
        5,
    )
...
def growing_tree_old_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).growing_tree(mode="oldest"), number=1, globals=globals()
        ),
        5,
    )
...
def growing_tree_rand_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).growing_tree(mode="random"), number=1, globals=globals()
        ),
        5,
    )
...
def growing_tree_mixed_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).growing_tree(mode="mixed"), number=1, globals=globals()
        ),
        5,
    )
...
def aldous_broder_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).aldous_broder(), number=1, globals=globals()),
        5,
    )
...
def wilson_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(lambda: Maze(size).wilson(), number=1, globals=globals()), 5
    )
...
def recursive_backtracking_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: RecursiveMaze(size).recursive_backtracking(),
            number=1,
            globals=globals(),
        ),
        5,
...
def recursive_kruskal_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: RecursiveMaze(size).recursive_kruskal(), number=1, globals=globals()
        ),
        5,
    )
...
def recursive_hunt_and_kill_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: RecursiveMaze(size).recursive_hunt_and_kill(),
            number=1,
            globals=globals(),
        ),
        5,
...
def iterative_division_time(size: int, queue: Queue) -> None:
    time = round(
        timeit.timeit(
            lambda: Maze(size).iterative_division(), number=1, globals=globals()
        ),
        5,
    )
...
def time_complexity() -> None:
    ...
    for size in tqdm(range(max_size + 1, 5)):
        ...
        for func in [
            kruskal_time,
            prim_time,
            randomized_depth_first_search_time,
            hunt_and_kill_time,
            binary_tree_time,
            recursive_division_time,
            eller_time,
            sidewinder_time,
            growing_tree_new_time,
            growing_tree_mid_time,
            growing_tree_old_time,
            growing_tree_rand_time,
            growing_tree_mixed_time,
            aldous_broder_time,
            wilson_time,
            recursive_backtracking_time,
            recursive_kruskal_time,
            recursive_hunt_and_kill_time,
            iterative_division_time,
        ]:
            queue = Queue()
            queues.append(queue)
...
def memory_complexity() -> None:
    ...
    for size in tqdm(range(max_size + 1, 5)):
        ...
        for func in [
            kruskal_memory,
            prim_memory,
            randomized_depth_first_search_memory,
            hunt_and_kill_memory,
            binary_tree_memory,
            eller_memory,
            recursive_division_memory,
            sidewinder_memory,
            growing_tree_new_memory,
            growing_tree_mid_memory,
            growing_tree_old_memory,
            growing_tree_rand_memory,
            growing_tree_mixed_memory,
            aldous_broder_memory,
            wilson_memory,
            recursive_backtracking_memory,
            recursive_kruskal_memory,
            recursive_hunt_and_kill_memory,
            iterative_division_memory,
        ]:
            queue = Queue()
            queues.append(queue)
            p = Process(target=func, args=(size, queue))
