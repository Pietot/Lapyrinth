...
def time_complexity() -> None:
    ...
    for size in tqdm(range(0, max_size + 1, 50)):
        ...
        maze = Maze(size)
        maze.sidewinder()
        for func in [left_hand_time, right_hand_time, random_mouse_time, pledge_left_time,
                     pledge_right_time, dead_end_filler_time, depth_first_search_time,
                     breadth_first_search_time, greedy_best_first_search_time, a_star_time]:
            queue = Queue()
            queues.append(queue)
    ...
    with open('pathfinder_time_complexity.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)

        writer.writerow(['Size', 'Left Hand', 'Right Hand', 'Random Mouse', 'Pledge Left',
                         'Pledge Right', 'Dead End Filler', 'Depth First Search',
                         'Breadth First Search', 'Greedy Best First Search', 'A*'])
...
def memory_complexity() -> None:
    ...
    for size in tqdm(range(0, max_size + 1, 50)):
        ...
        maze = Maze(size)
        maze.sidewinder()
        for func in [left_hand_memory, right_hand_memory, random_mouse_memory, pledge_left_memory,
                     pledge_right_memory, dead_end_filler_memory, depth_first_search_memory,
                     breadth_first_search_memory, greedy_best_first_search_memory, a_star_memory]:
            queue = Queue()
            queues.append(queue)
            process = Process(target=func, args=(maze, size, queue))
            processes.append(process)
    ...
    with open('pathfinder_memory_complexity.csv', 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
